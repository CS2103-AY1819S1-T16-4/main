...
Internally, `ModuleInfo` is used to hold the data for a module such as name, description, module code, prerequisites and preclusions.
`MajorDescription` is used to hold data for a major such as name, prefixes of its modules and a list of modules.

.Class diagram for `ModuleInfo`.
image::ModuleInfoClassDiagram.png[width="650"]

.Class diagram for `MajorDescription`.
image::MajorDescriptionClassDiagram.png[width="800"]
...
* **Alternative 1**: parse the sentences with natural language processing to extract the correct prerequisite and preclusion relationship of the modules
** Pros: Prerequisite and preclusion relationship of our data will be correct.
** Cons: Difficult to automate with a script due to the potential long list of exception rules.

* **Alternative 2 (Current approach)**: Extract the module codes with regular expression and assume AND relationship for prerequisites.
** Pros: Extraction can be done easily with regular expression. Prerequisite and preclusion checking is also simple to implement in Java code.
** Cons: The final data will be incorrect for rules like "(MA1101R AND MA1221) OR MA1521". How

Note that corequisite relationship that descibes which modules must be taken together is not yet implemented
as it will complicate the implementation of `add` and `delete` commands.
...
* **Alternative 2**: Generate Java code during build time. The data will be stored as a static constant Java object.
...
===== Aspect: Retrieval of module and major information

Since we have decided to take alternative 1 in <<Aspect: Storing of module and major information>>, we need
a way to 
* **Alternative 1**: Implement a `ModuleInfoStorage` and `MajorDescriptionStorage`.
** Pros: Since we are deserializing from file content, it is reasonable to implement the retrieval logic under storage component.
** Cons: We only need the ability to read from file and not writing into file for the implementation. Even the implementation of 
file reading is very different

//// Comment out
==== Future code health improvements

* Use a new class `JsonAdaptedModuleInfo` for deserialization and use `JsonAdaptedModuleInfo#toModelType` to
construct the actual type. This will allow us to remove `ModuleInfo#finalizeModuleInfo` that needs to be called
after `ModuleInfo` is deserialized from JSON resource file.
* Consolidate `ModuleType` and `DegreeRequirement` into the later enum as it is closer to the real world.
* Merge `Module` and `ModuleInfo` into one class. They are separate classes as it was initially thought that `ModuleInfo`
will hold all the immutable data about a module and `Module` will 
* Remove the use of `Module#getType` (currently only used by UI and tests).
////
...
are collected and made known to the user through the Result Display. They are separated by their types:
