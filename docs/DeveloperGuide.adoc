...
.Interactions inside the Storage component whenever `ModulePlannerChangedEvent` is fired.
...
This gives us a reasonable approximation of prerquisite and preclusion relationship for most modules.
...
NUS sometime creates new modules to replace outdated modules every academic year. Information about
outdated modules are not given by IVLE, but other modules might still list their module codes in prerequisites
...
** Cons: Need to explicitly list down all the possible `enum` values (there are more than one hundred focus areas
across all NUS academic majors). Special JSON serialization and deserialization is needed for enum types.
...
the parameter is a valid academic major/focus area. This leads to a lot of redundant checking.
...
* **Alternative 1**: Execute syntax format checking one by one and immediately throw command exception in first failure.
...
* **Alternative 2 (current choice)**: Always execute all syntax format checking. If there are more than one error, concatenate their error
