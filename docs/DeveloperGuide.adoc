...
`ResultDisplay`, `PersonListPanel`, `StatusBarFooter`, `BrowserPanel` etc.
...
* can save the Module Planner data in xml format and read it back.
...
*'ModulePlanner#addModules(List<Module> modules, int index)' -- Add the list of modules 'modules' into the semester specified by 'index'.
...
[NOTE]
A valid index should be an integer between 1 to 8 inclusive, where index 1 represents year 1 semester 1, index 2 represents year 1 semester 2, and so on.
...
Step 2. The user executes `add y/1 s/1 CS1231`. The `add` command inserts `Module` CS1231 to the list `modulesTaken` for `Semester` object with index 1 (stored in index 0 of `semesters`).

Step 3. The user wants to see the list of modules for year 1 semester 1 by executing `list y/1 s/1`. The `list` command parses `y/1 s/1` to index 1 and calls `Model#getFilteredTakenModuleList(int index)`.
