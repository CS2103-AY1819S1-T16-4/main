...
* Consolidate `ModuleType` and `DegreeRequirement`.
* Merge `Module` and `ModuleInfo` into one class and remove the use of `Module#getType` (currently only used by UI).

===== Aspect: Prerequisites and preclusions
