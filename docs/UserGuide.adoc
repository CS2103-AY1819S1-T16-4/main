...
|MAJOR | Supported majors are listed in `setup` and `suggest`.

|FOCUS_AREA | Supported focus areas are listed in `setup` and `suggest`.

|MODULE_CODE | Must be valid NUS module code in Academic Year 18/19.

|YEAR | Should only be integer from `1` to `4`.

|SEMESTER | Should only be integer `1` or `2`.
