...
[NOTE]
From the input list of modules, a subset of it which will be added to the planner does not contain modules which falls in one of the following criterias: +
1. not offered Modules +
2. Modules not in the planner +
3. Modules which one of its preclusion exist in the planner +
4. Two or more modules equivalent (preclusion) to each other +
...
Shows how many core and general education modules that you have taken and how many that you need to take. +
Format: `status`
