...
If a valid year is supplied, shows a list of all modules that you have taken (added using `add` command) in that year. Otherwise, shows a list of all modules that you have taken in every year. +
Format: `list [y/YEAR]`
...
[NOTE]
====
* If no modules have been taken in the specified year or no modules have been taken in any year, will show an empty list.
====
