...
=== Major enhancement
* Implement mechanism to retrieve module and major information
** What it does: Collect and preprocess module and major information from https://nusmods.com[NUSMods],
...
information about NUS modules and majors to function.
** Highlight:
*** This requires a clear understanding of the types of information about modules and majors
and how they relate with each other so that they can be represented as well-designed classes.
*** Scripting experience is needed to preprocess the data from external sources so that it can be easily
manipulated in application code.
*** Need to strike a balance between the potential implementation complexity of other commands and how
accurate the processed data should be when compared with the real world data so that it is closer
to NUS students' expectation.
...
* Co-author `setup` command with Gabriel Yik to initialize user's major and focus areas.
